...
from systems import HarmonicOscillator, PoschlTeller, BaseSystem
from utils import MetropolisSampler, RandomSampler, energy
...
parser.add_argument('--system', type=str, default='harmonic', choices=['harmonic', 'pt'], help='which quantum system to run')
parser.add_argument('--dim', type=int, default=1, choices=range(1, 4), help='coordinate space dimensions')
parser.add_argument('--N', type=int, default=3000, help='number of coordinates to sample')
...
def train(
    ...
    r0 = torch.zeros(1, 1, system.dim) # initial fixed point
...
N = args.N
sampler =RandomSampler(system=oscillator, N=N)
data = sampler.sample()
...
torch.save(model.state_dict(), f'/Users/Ameya/code/Quantum-Potential-Prediction/saved-models/{args.system}{args.dim}d.pth')
with open(f'/Users/Ameya/code/Quantum-Potential-Prediction/saved-models/{args.system}{args.dim}d-metrics.p', 'wb') as fp:
